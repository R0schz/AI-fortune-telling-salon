...
export function Home() {
  ...
  const handleDivination = async () => {
    try {
      // プレミアムユーザーまたは非ログインユーザーはチケット不要
      if (state.user?.is_premium || !state.isLoggedIn) {
        generateResult();
      ...
      const needsTicket = !isPersonalFortune || state.tickets?.daily_free_used;
      ...
      if (isPersonalFortune && !state.tickets?.daily_free_used) {
  ...
  const generateResult = () => {
    ...
    const mockResults = {
      tarot: `${tonePrefix}「恋人」のカードが出ました。今日は新しい出会いや関係性の深まりが期待できる日です。心を開いて、素敵な縁を大切にしてくださいね。真実の愛があなたを待っています。`,
      numerology: `${tonePrefix}あなたの運命数は「7」です。直感力が冴えている時期なので、ひらめきを大切にしてください。内なる声に耳を傾けることで、正しい道が見えてきます。瞑想や静かな時間を作ることをお勧めします。`,
      astrology: `${tonePrefix}金星が良い位置にあります。愛情運が上昇中で、素敵な一日になりそうです。積極的にコミュニケーションを取ることで、良い流れを引き寄せられるでしょう。特に午後の時間帯が幸運です。`,
      crystalball: `${tonePrefix}水晶に美しい光が見えます。近いうちに嬉しいニュースが舞い込むでしょう。その知らせは、あなたの努力が実を結んだ証です。希望を持ち続けることで、夢が現実となります。`
    };
  ...
  const handleSaveResult = async () => {
    if (!state.isLoggedIn) {
      alert('占い結果を保存するにはログインが必要です。マイページからログインしてください。');
      return;
    }
    ...
    try {
      ...
      alert('占い結果を保存しました！マイページから確認できます。');
    } catch (error) {
  ...
  const isDailyFreeUsed = state.isLoggedIn ? (state.tickets?.daily_free_used || false) : false;
  const isPremium = state.user?.is_premium || false;
          ...
          <div className="space-y-4">
            ...
            >
              <CardHeader className="text-center">
                ...
                {!state.isLoggedIn ? (
                  <Badge variant="secondary" className="w-fit mx-auto mt-2 bg-blue-100 text-blue-800">
                    無料体験
                  </Badge>
                ) : !isDailyFreeUsed && !isPremium ? (
      ...
      {currentStep === 'input' && selectedMethod && (
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            {/* チケット消費の案内 */}
            {state.isLoggedIn && !isPremium && (
              <>
                {selectedPurpose === 'personal' && isDailyFreeUsed && (
                  <Alert>
          ...
          <div className="space-y-2">
            ...
            {!state.isLoggedIn && (
              <Alert>
                <Info className="h-4 w-4" />
                <AlertDescription>
                  ログインすると占い結果を保存して、いつでも見返すことができます。
                </AlertDescription>
              </Alert>
            )}
          </div>
